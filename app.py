"""
FinSight – Financial Analytics for Interview Preparation
BA870/AC820 Final Project · Spring 2026
Team: Do Hyun Chun, Xinyi Xu, Yunheng Lu
"""
...
if not history.empty and len(history) >= 252:
    p_now = history["Close"].iloc[-1]
    p_1y  = history["Close"].iloc[-252]
    ret_1y = (p_now - p_1y) / p_1y * 100
elif not history.empty and len(history) > 1:
    p_now = history["Close"].iloc[-1]
    p_1y  = history["Close"].iloc[0]
    ret_1y = (p_now - p_1y) / p_1y * 100
...
st.divider()
st.caption("→ Use the sidebar to navigate: KPI Dashboard · Trend Analysis · Peer Benchmarking · Risk Analysis · News & Filings")
